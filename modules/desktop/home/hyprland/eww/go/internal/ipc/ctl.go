...
// sendControlCommand returns the daemon's reply verbatim; re-marshalling it
...
func sendControlCommand(p pyjson.Payload) (string, error) {
	...
	if unixConn, ok := conn.(*net.UnixConn); ok {
		// Half-close: the daemon reads until EOF or newline, so this lets it use either.
...
func Run(args []string) int {
	...
	switch {
	case err != nil:
		// Every failure is wrapped as a failed response rather than a traceback: all 25
		// eww-barctl call sites in eww.yuck depend on the shape, the exit code and the
		// --quiet silence.
